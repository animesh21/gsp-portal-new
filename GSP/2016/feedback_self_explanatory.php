...
if($state1=="All India")
	 {
		...
		$state_condition=" AND lsur.`attribute_5`='".$state1."'";
...
$self_explanatory_yes = mysql_query("SELECT * FROM lime_survey_266617 AS ltok JOIN `lime_tokens_266617` AS lsur ON ltok.token=lsur.token WHERE 
lsur.`completed` !='N' AND lsur.`usesleft` <= 0 AND ltok.`submitdate` IS NOT NULL  AND ltok.266617X39X2432='Y' $state_condition");
$selfexplanatoryresult_yes = mysql_num_rows($self_explanatory_yes); 


$self_explanatory_no = mysql_query("SELECT * FROM lime_survey_266617 AS ltok JOIN `lime_tokens_266617` AS lsur ON ltok.token=lsur.token WHERE 
lsur.`completed` !='N' AND lsur.`usesleft` <= 0 AND ltok.`submitdate` IS NOT NULL  AND ltok.266617X39X2432='N' $state_condition");
$selfexplanatoryresult_no = mysql_num_rows($self_explanatory_no); 
...
$(function () {
    ...
    $(document).ready(function () {
        ...
        $('#container').highcharts({
            ...
            title: {
                text: 'Was the Audit easy to understand and self explanatory? <?php echo $statename; ?>'
            },
            tooltip: {
        ...
        plotOptions: {
            ...
            series: [{
                colorByPoint: true,
                data: [{name: 'Yes', y: <?php echo $selfexplanatoryresult_yes; ?>}, {name: 'No', y: <?php echo $selfexplanatoryresult_no; ?>}]
            }]
        });
    });
}); 
...
<div class="top-area">
      	<h2>FEEDBACK BY STATE (Was the Audit easy to understand and self explanatory?)</h2>
</div>
    <form name="state_compare" id="state_compare" method="post">
        ...
        <select id="state1" class="form-control" name="state1">
            ...
            <option value="All India" <?php if($_REQUEST['state1'] =="All India") { ?>selected="selected"<?php } ?>>All India</option>
            <?php
