...
<ul class="filter_style">
   ...
   <li><label>Select State</label> 
       <select class="form-control" id="region_state" name="region_state">
           <option value="0">All India</option>
            <?php
            $sql_zones_states = mysql_query("select * from `states_india` where `rid` = '".$_POST['region']."'");
            while($zones_states_array = mysql_fetch_array($sql_zones_states))
            {
            ?>
            <option value="<?php echo $zones_states_array['state_id']; ?>"<?php if($_POST['region_state'] == $zones_states_array['state_id']) { ?> selected="selected" <?php } ?>><?php echo $zones_states_array['name']; ?></option>
            <?php } ?>
        </select>
   </li>
   </div>
   ...
   <li><label>Select School Parameter</label>
       <div style="overflow: hidden; clear: both;">
           <select class="form-control" name="category">
			...
			<option value="4" <?php if($_POST['category'] ==4) { ?> selected="selected" <?php } ?>>Day Scholar + Day Boarding</option>
			<option value="5" <?php if($_POST['category'] ==5) { ?> selected="selected" <?php } ?>>Day Boarding + Residential</option>
			<option value="6" <?php if($_POST['category'] ==6) { ?> selected="selected" <?php } ?>>Day Scholar + Residential</option>
			<option value="7" <?php if($_POST['category'] ==7) { ?> selected="selected" <?php } ?>>Day Scholar + Day Boarding + Residential</option>
         </select>
