...
<main class="bdr" style='border: none;'>
    <div class="container" style="width: 100%">
     ...
     while($states_array = mysql_fetch_array($sql_states))
     {
        ...
        $sql_waterconsumption = mysql_fetch_assoc(mysql_query("SELECT sum(greencount.266617X37X1883) as waterconsumption FROM `lime_survey_266617` as greencount,lime_tokens_266617 as takesytate where greencount.token=takesytate.token AND takesytate.usesleft <=0 AND takesytate.completed !='N' AND takesytate.attribute_5='".$states_array['state_id']."'"));
		
		
		 $sql_Total_population = mysql_fetch_assoc(mysql_query("SELECT sum(greencount.266617X32X33SQ004_SQ003) as total_population FROM `lime_survey_266617` as greencount,lime_tokens_266617 as takesytate where greencount.token=takesytate.token AND takesytate.usesleft <=0 AND takesytate.completed !='N' AND takesytate.attribute_5='".$states_array['state_id']."'"));
