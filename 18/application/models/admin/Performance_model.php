...
return $this->db->select('a.*,b.name AS state_name, c.name AS district_name')->from('tbl_total AS a')
	...
	public function getExcelPrimarySchool()
	 ...
	 
		{
			...
			return $this->db->select('a.*,b.userid,b.questionid,b.answer,c.name,c.id,d.category AS category,d.population ,e.name AS state_name, f.name AS district_name, c.schoolemail AS schoolemail')
							->from('tbl_total AS a')	
			                ->join('gsp_answers As b','a.userid=b.userid','left')
							->join('gsp_school AS c','b.userid=c.userid', 'left')
                 			->join('tbl_general AS d', 'c.userid=d.userid', 'left')
