...
WHERE lsur.`completed` !='N' AND lsur.`usesleft` <= 0 AND ltok.`submitdate` IS NOT NULL  AND  ltok.266617X39X2426='Y' $state_condition");
...
WHERE lsur.`completed` !='N' AND lsur.`usesleft` <= 0 AND ltok.`submitdate` IS NOT NULL AND ltok.266617X39X2426='N' $state_condition");
...
$(function () {
    ...
    $(document).ready(function () {
        ...
        $('#container').highcharts({
            ...
            title: {
                text: 'Has your school participated in the Green Schools Programme (GSP) Audit before? <?php echo $statename; ?>'
            },
            tooltip: {
        ...
        plotOptions: {
            ...
            series: [{
                colorByPoint: true,
                data: [{name: 'Yes', y: <?php echo $totalresource_efficient_yes; ?>}, {name: 'No', y: <?php echo $totalresource_efficient_no; ?>}]
            }]
        });
    });
}); 
...
<div class="top-area">
      	<h2>FEEDBACK BY STATE (Has your school participated in the Green Schools Programme (GSP) Audit before?)</h2>
</div>
