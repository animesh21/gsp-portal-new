...
$sql_email_school = mysql_fetch_array(mysql_query("SELECT `email`, `attribute_1`, `attribute_12`, `tid` FROM lime_tokens_266617 WHERE `token` = '".$sql_token_details."'"));   
...
/////////////////////////////////////////////// MAIL SCRIPT ///////////////////////////////////////////////////////////////////////
    if($sql_email_school['attribute_12'] !="")
    {
    $to = $sql_email_school['email'].",".$sql_email_school['attribute_12'];
    }
    else
    {
    $to = $sql_email_school['email'];    
    }
    //$cc = $sql_email_school['attribute_12'];
    $from = "support@greenschoolsprogramme.org";
    $subject = "Your Green School Programme Responses";
    $message = "Please find the attached PDF file along with this mail.";        
    $fname=$postfix;
...
// boundary 
    ...
    if($ok)
    {
        ...
        $msg = 'Response PDF of ';
        $msg .=$sql_email_school['attribute_1'];
        $msg .=' has been successfully sent to ';
        $msg .=$sql_email_school['email'];
        $msg .=' and ';
        $msg .=$sql_email_school['attribute_12'];
        echo '<div style="text-align: center; color: green; font-size:16px; margin-top:20px;">';
        echo $msg;
        echo '</div>';
    }
    //$url = $_SERVER['HTTP_REFERER'];
    //header('location:'.$url.'?para=success');
?>
