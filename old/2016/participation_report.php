...
<!--     <th>School Parameter</th>-->
     ...
     elseif($_REQUEST['region'] != 0 && $_REQUEST['region_state'] != 0)
		$where = " where rid = '" . $_REQUEST['region']."' AND state_id = '". $_REQUEST['region_state']."'";
	 ...
	 if ($_REQUEST['category'] != 0)
	 {
		 ...
		 {
			 $whereClause[] = "ls.266617X34X1608 = '1'";
		 }
		 elseif($_REQUEST['category'] == 2)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '2'";
		 }
		 elseif($_REQUEST['category'] == 3)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '3'";
		 }
		 
		  elseif($_REQUEST['category'] == 4)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '4'";
		 }
		 
		  elseif($_REQUEST['category'] == 5)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '5'";
		 }
		  elseif($_REQUEST['category'] == 6)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '6'";
		 }
		 
		  elseif($_REQUEST['category'] == 7)
		 {
			 $whereClause[] = "ls.266617X34X1608 = '7'";
		 }
     }
	  ...
	  if ($_REQUEST['board'] != 0)
	 {
		 if($_REQUEST['board'] == 1)
		 {
			 $whereClause[] = "ls.266617X32X28 = '1'";
		 }
		 elseif($_REQUEST['board'] == 2)
		 {
			 $whereClause[] = "ls.266617X32X28 = '2'";
		 }
		 
		 elseif($_REQUEST['board'] == 3)
		 {
			 $whereClause[] = "ls.266617X32X28 = '3'";
		 }
		 
		 elseif($_REQUEST['board'] == 4)
		 {
			 $whereClause[] = "ls.266617X32X28 = '4'";
		 }
	...
	if(!empty($whereClause))
     ...
     $sql_states = mysql_query("select `state_id`,`name` from $table1 $where");
	 
	 // loop through states and output
     while($data = mysql_fetch_array($sql_states))
     {   
		// get total registrations for the state
		$query = "SELECT count(token) FROM lime_tokens_266617  where attribute_5='" . $data['state_id'] . "'";
		...
		$total_reg_count = mysql_result($total_reg_count_data, 0, 0);
		 ...
		 // Count audit started for particular state
		  $query_audit="SELECT count(*) FROM `lime_tokens_266617` as lt JOIN `lime_survey_266617` as ls on lt.token = ls.token where attribute_5='" . $data['state_id'] . "' $whereClause";
		 ...
		 //echo "<br>";
		 $query_audit_completed="SELECT count(*) FROM `lime_tokens_266617` as lt JOIN `lime_survey_266617` as ls on ls.token = lt.token where lt.completed !='N' AND lt.usesleft <= 0 AND ls.submitdate IS NOT NULL AND lt.attribute_5='" . $data['state_id']."' $whereClause";
